...
while iterations < max_iterations:
    ...
    if 2300 < Re_t_p < 1e6:
        ...
        print(
            f"If you change the temperatures, you will need to change thermal properties accordingly"
        )
        print(
            f"Or you can change any of the mass flowrate as well, the other mass flowrate will be affected by it"
        )
    ...
    Uo_new = 1 / (
        Ao * (1 / (hi * Ai) + Wall_resistance + 1 / (ho * Ao) + Rf_i / Ai + Rf_o / Ao)
    )
    ...
    if (
        abs(Ts_out_new - Ts_out) < convergence_limit
        and abs(Tt_out_new - Tt_out) < convergence_limit
        and abs(Uo_new - Uo) < convergence_limit
    ):
        break
...
while iterations < max_iterations:
    ...
    # Append temperature values for plotting
    Ts_inlet_values.append(Ts_in)
    Ts_outlet_values.append(Ts_out)
    Tt_inlet_values.append(Tt_in)
    Tt_outlet_values.append(Tt_out)
...
plt.plot(iterations_axis, Ts_inlet_values, label="Ts Inlet")
plt.plot(iterations_axis, Ts_outlet_values, label="Ts Outlet")
plt.plot(iterations_axis, Tt_inlet_values, label="Tt Inlet")
plt.plot(iterations_axis, Tt_outlet_values, label="Tt Outlet")

plt.xlabel("Iterations")
plt.ylabel("Temperature (°C)")
plt.title("Temperature Profiles during Iterations")
plt.legend()
plt.show()
